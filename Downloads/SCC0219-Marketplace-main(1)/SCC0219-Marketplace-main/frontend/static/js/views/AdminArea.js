...
//    section.classList.add("col", "s12", "hide");
    section.id = "addprod";
  ...
  async mountUpdateProduct() {
    ...
    let produtos = {};
    produtos = await fetch('http://localhost:8080/product', options).then(T => T.json()).then(response => {
      if(response != null) {
        // console.log(response)
        return response
      }
     }).catch(() => alert("Não foi possível encontrar usuarios"));

    
    for (const key in produtos) {
      let inputOption = document.createElement("option");
      inputOption.value = produtos[key]._id;
      inputOption.innerHTML = "ID: " + produtos[key]._id + " Nome: " + produtos[key].name;
  ...
  async mountListUser() {
    ...
    let usuarios = {};
    usuarios = await fetch('http://localhost:8080/user', options).then(T => T.json()).then(response => {
      if(response != null) {
        // console.log(response)
        return response
      }
    ...
    for (const key in usuarios) {
      ...
      const user = "Nome: " + usuarios[key].name + "<br>Email: " + usuarios[key].email
      + "<br>Senha: " + usuarios[key].password + "<br>Role: " + usuarios[key].role + 
      "<br>Telefone: " + usuarios[key].phone + "<br>Endereco: " + usuarios[key].address
  ...
  async getHtml() {
    ...
    // linkAddProd.addEventListener("click", () => document.querySelector("#addprod").classList.remove("hide"));
